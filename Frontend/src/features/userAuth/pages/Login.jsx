    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                seterror({ password: result.message || "Registration failed" });
            }
        } catch (err) {
            console.log(err);
            seterror({ password: "Something went wrong. Try again." });
        }
    }
    ...
    return (
        <main className="min-h-screen w-full flex justify-center items-center bg-gradient-to-br from-gray-900  px-4">
            <div className="w-full max-w-md bg-white/90 backdrop-blur-md p-6 sm:p-8 rounded-2xl shadow-xl flex flex-col gap-5">
                <h1 className="text-2xl sm:text-3xl font-bold text-center text-gray-800">Login </h1>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <button className="mt-2 px-6 cursor-pointer py-3 rounded-2xl bg-blue-500 text-white text-base sm:text-lg font-bold hover:bg-blue-400 transition-all duration-300"> Login </button>
                </form>
                <p className=" text-center  text-gray-800">Account don't exists? <Link className="text-red-600" to={"/register"}>SignUp</Link></p>
            </div>
        </main>
    )
}
